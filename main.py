...
class soda():
    def __init__(self):
        ...
        threading.Thread(target=self.smartBHClicking, daemon=True).start()
    ...
    def smartBHClicking(self):
        while True:
            if(self.config["left"]["enabled"] and win32api.GetAsyncKeyState(self.config["left"]["smartBH"]) != 0 and self.isFocused("left", "onlyWhenFocused", "workInMenus")):
                self.leftClick(True)
                time.sleep(0.5);
